            ...
            <div className="flex flex-col justify-center items-center gap-[40px] text-center text-forground w-full">
                <Accordion type="single" collapsible className="w-full flex flex-col gap-[40px]">
                    {components.map((component) => (
                        <BlurFade key={component.title} delay={0.3} inView>
                            <AccordionItem value={component.title}  className="flex flex-col justify-center items-center p-[20px] gap-[10px] bg-design rounded-xl border-none">
                                    ...
                                    <div className="flex flex-wrap justify-center items-center gap-[10px] md:gap-[40px]">
                                        {images.map((image) => (
                                            <div key={image.name} className="flex flex-col justify-end items-center relative rounded-[20px]"> 
                                                <Image key={image.src} src={image.src} alt={image.alt} width={200} height={200} className="w-[146px] h-[162px] md:w-[280px] md:h-[310px] rounded-[20px] "/>
